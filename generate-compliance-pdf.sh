...
if docker build -f compliance/Dockerfile . -t gcp-compliance ; then
...
# build the PDF
docker run --rm -it --entrypoint="" -v "$(pwd)"/compliance/exports/pdf:/app/compliance/exports/pdf gcp-compliance gitbook pdf . ./pdf/gcp_compliance.pdf
...
if [ "$1" = "-w" ] ; then
	...
	docker run --rm -i -t -p 4000:4000 gcp-compliance
fi
